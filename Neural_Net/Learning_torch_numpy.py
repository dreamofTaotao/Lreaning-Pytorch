...
for it in range(500):
    ...
    #接↑，在计算矩阵的相关问题的时候，我们可以进行反向推导，因为矩阵的运算点积的行列位置是不能进行互换的
    #所以，结果是固定的，那么就可以推导出来谁点积谁了，例如：(这个是矩阵的相关运算)
    #上面的grad_w2【100*10】 = h_relu【64*100】.T【转置】.dot(grad_y_pred【64*10】)
    #现在解释一下，需要在反向传播的过程中求谁的倒数的问题
    #运用链式求导法则进行理解，注意运用上面的简易的约分法则。
    #举一个简单的例子（运用高数书中的知识）：
    #f(g(k(x))) = (df / dg) * (dg / dk) * (dk / dx) = df / dx
    #这样应该就了解了
    
    #update weights of w1 and w2
    #这里呢就是将我们求得的权值的倒数的值进行更新
